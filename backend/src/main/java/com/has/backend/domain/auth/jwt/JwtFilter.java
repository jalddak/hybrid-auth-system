...
import com.has.backend.domain.auth.util.ResponseUtil;
...
public class JwtFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (accessTokenRepository.isBlacklisted(accessToken)) {
            ...
            ResponseUtil.setResponse(response, HttpStatus.UNAUTHORIZED, false, "Invalid access token.");
            return;
        }
        ...
        try {
            ...
            ResponseUtil.setResponse(response, HttpStatus.UNAUTHORIZED, false, "NEED_REISSUE");
            return;
        ...
        if (!"access".equals(claims.get("category", String.class))) {
            ...
            ResponseUtil.setResponse(response, HttpStatus.UNAUTHORIZED, false, "Not an access token.");
            return;
        }
        ...
        boolean invalidated = accessTokenRepository.findInvalidationTime(username)
                .map(invalidatedAtMs -> issuedAtMs < invalidatedAtMs)
                .orElse(false);

        if (invalidated) {
            log.error("Access token invalidated. username={}", username);
            ResponseUtil.setResponse(response, HttpStatus.UNAUTHORIZED, false, "Invalid access token.");
            return;
        }

        CustomUserDetails customUserDetails = CustomUserDetails.forJwt(username, userId);
